...
def search_db(db_in = 'swissprot', query_in = 'fakefastaJawn', results_out = 'results.out'):
	...
	search_database = subprocess.run(["blastp", "-db", db ,"-query", query, "-out", results, "-outfmt", "6 sseqid bitscore staxids"])
	return 0

# def read_output(results = 'results.out', min_score = 800):
# 	fid_user = open('UserHistory.txt','a')
# 	fid = open(results,'r')
# 	arr = fid.readline()
# 	arr_list = arr.split()
...
# def nosy (UserHistory = 'UserHistory.txt'):
# 	fid = open(results,'r')
